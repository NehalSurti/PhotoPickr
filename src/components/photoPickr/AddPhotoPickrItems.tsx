...
import { PHOTOPICKR_ITEMS_URL } from "@/lib/apiEndPoints";
import { toast } from "sonner";
import { useRouter } from "next/navigation";

export default function AddPhotoPickrItems({
  token,
  photoPickrId,
}: {
  token: string;
  ...
  const [items, setItems] = useState<Array<PhotoPickrItemForm>>([
    { image: null },
    { image: null },
  ]);
  ...
  const handleSubmit = async () => {
    try {
      ...
      formData.append("id", photoPickrId);
      items.map((item) => {
        if (item.image) formData.append(`images[]`, item.image);
      });
      if (formData.get("images[]")) {
        setLoading(true);
        const { data } = await axios.post(PHOTOPICKR_ITEMS_URL, formData, {
          headers: {
            Authorization: token,
          },
        });
